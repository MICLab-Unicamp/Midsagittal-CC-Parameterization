...
with st.container(border=True):
    ...
    if not data_path.strip():
        ...
        st.error("Dataset path is not a directory.")
        configuration_valid = False
    else:
    ...
    if st.session_state.csv_file:
        ...
        if not csv_path.strip():
            st.error("CSV path cannot be empty.")
            configuration_valid = False
        elif not csv_file.exists():
            st.error("CSV file not found.")
            configuration_valid = False
        elif not csv_file.is_file():
            st.error("CSV path is not a file.")
            configuration_valid = False
        else:
            try:
...
def click_button():
    ...
    for i,sid in enumerate(valid_subjects):
        ...
        if st.session_state.save_npy_sep:
            ...
            sub_path_param = os.path.join(sub_path, "param")
    ...
    if st.session_state.save_default or st.session_state.save_nii_sep:
        ...
        if st.session_state.save_nii_sep:
            for sid in dict_param_points:
                sub_path_param = os.path.join(data_path, sid, "param")
                os.makedirs(sub_path_param, exist_ok=True)
