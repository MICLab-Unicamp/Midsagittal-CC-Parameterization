...
from lib_param.util import cut_imgs_mask,vis_param,vis_param_cc
...
data_path = st.text_input("Dataset path (should contain one folder per individual, each with the required .nii or .nii.gz files):",
                          "/home/caio/nas_caio/Datasets_param_application/age_sex_analysis/paper_6/test_data")
...
def click_button():
    ...
    n_subs = len(glob(os.path.join(data_path, '*/')))
    ...
    for i,sub_path in enumerate(sorted(glob(os.path.join(data_path, '*/')))):
        ...
        if st.session_state.save_npy_sep:
            ...
            np.save(os.path.join(sub_path, f"data_param_{sid}"), data_param)
    ...
    if st.session_state.save_default:
        ...
        np.save(os.path.join(data_path, "data_param"), data_param)
